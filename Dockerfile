...
# The standalone launcher derives the runtime heap from the tightest current
# host/cgroup/process-available budget before it starts the actual server Node
# process. OMNIROUTE_MEMORY_MB and an operator-supplied NODE_OPTIONS heap retain
# precedence; the image must not pin either knob and bypass that calibration.
